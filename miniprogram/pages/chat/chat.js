...
Page({
  ...
  onLoad(q) {
    ...
    // 首次进入（无历史会话）：显示欢迎语，让用户一眼确认页面已就绪、可输入
    // 之前空白对话区+白色输入栏融入浅蓝渐变，真机看上去像「页面没加载出来」
    if (!this.data.sessionId && this.data.messages.length === 0) {
      this.addMsg('bot', '你好呀～我是你的花艺小助手 🌸\n想做一束花送给谁呢？告诉我用途（生日、表白、开业……）、大概预算和喜欢的风格或花材，我帮你设计专属花束，还能直接匹配附近花店下单哦。');
    }
  },
