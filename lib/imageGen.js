...
const { buildImagePrompt, buildImagePromptZh, buildNegativePrompt } = require('./imagePrompt');
...
// 通义万象：异步任务 + 轮询结果
async function callDashScope(prompt, size, negativePrompt) {
...
async function generate(plan, req) {
  ...
  const negativePrompt = buildNegativePrompt(plan);
  const size = isDashScope ? '1024*1024' : '1024x1024';

  if (process.env.IMAGE_API_KEY && isDashScope) {
    try {
      const res = await callDashScope(prompt, size, negativePrompt);
      return { ...res, prompt, negative_prompt: negativePrompt };
    } catch (e) {
