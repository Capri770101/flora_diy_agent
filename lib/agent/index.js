...
const { chatReplyFor } = require('./chat');
...
// 收尾：模板生成回复 → LLM 可用则润色为自然对话（失败回落模板）→ 记录 assistant 轮次
async function finalizeReply(ctx, role, partial) {
  const reply = await chatReplyFor({
    role,
    ctx: { requirements: ctx.requirements, plan: partial.plan || null, shops: partial.shop_suggestions || [], transcript: ctx.transcript },
    templateReply: partial.reply
  });
...
async function runAgent({ text, session, location, config }) {
  ...
  const reply = await chatReplyFor({
    role: 'plan',
    ctx: { requirements, plan, shops: shop_suggestions, transcript: ctx.transcript },
    templateReply
  });
  sessionStore.pushTurn(ctx, 'assistant', reply);
