...
// ④ 让方案"更懂行"：从新增域提炼与本次需求相关的洞察（潮流/地区/知识）
...
function buildDomainInsights(req, location, firstShopDistrict) {
  const insights = { trends: [], region: null, knowledge: [] };
  // F 市场潮流：按用户指定月份或当前月取高分花材
  const month = req.month || currentMonth();
  const trendRows = dataLayer.trendsAll().filter((t) => t.month === month).sort((a, b) => b.score - a.score).slice(0, 3);
  insights.trends = trendRows.map((t) => {
    const f = flowerKB.byId(t.flower_id);
    return { month: t.month, flower_id: t.flower_id, name: f ? f.name : t.flower_id, score: t.score };
  });
  // D 地区差异：优先用最近门店的 district 对应 region，否则按经纬度推断
  let region = firstShopDistrict ? dataLayer.regionsAll().find((r) => r.district === firstShopDistrict) : null;
  if (!region) region = regionByLocation(location);
  if (region) {
    insights.region = {
  ...
  // J 知识教育：按 occasion/recipient/intent 关键词匹配知识库
