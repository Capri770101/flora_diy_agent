...
// 对外：流式回复。onChunk(delta) 逐段回调 LLM 生成的文本；返回最终完整回复文本。
// LLM 不可用/超时/失败 → 不回调，一次性返回 templateReply（由调用方决定打字机动画）。
async function chatReplyStreamFor({ role, ctx, templateReply, onChunk }) {
  const system = buildSystemPrompt(role, ctx);
  const history = buildMessages(ctx);
  const out = await chatStreamReply({ system, history, temperature: 0.8, max_tokens: 400, onChunk });
  return out || templateReply;
}

module.exports = { chatReplyFor, chatReplyStreamFor, buildSystemPrompt, formatRequirements, formatPlan, formatShops };