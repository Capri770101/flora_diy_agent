...
function buildSummary(plan, req) {
  ...
  // 场合词取真实语义值；"其他"是规则兜底意图，写进摘要会显示成"其他花束"，必须剔除
  const occ = req.occasion && req.occasion !== '其他'
    ? req.occasion
    : (req.intent && req.intent !== '其他' ? req.intent : '');
  const cat = plan.category || '花束';
  return `一份${styleStr}风格${occ ? '的' + occ : ''}的${cat}：${names}，配${plan.package}。`;
}
