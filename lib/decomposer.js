...
function detectPreferred(text, flowers) {
  ...
  for (const f of flowers) {
    ...
    if (/(不要|别用|不用|忌|别放|少放|不放|不想要|不喜欢|不爱|讨厌|去掉)/.test(before)) continue;
...
function extractQuantities(text, flowers) {
  ...
  // 品类词后缀：带「花束/花盒」等的是开放设计请求，不算明确数量订单（避免误触发现算路径）
  const CATEGORY_TRAIL = ['花束', '花盒', '花篮', '捧花', '胸花', '花环'];
  const trailingCategory = (name) => CATEGORY_TRAIL.some((w) => name.endsWith(w));
  const reA = new RegExp('(\\d+)\\s*(' + units + ')?\\s*([\\u4e00-\\u9fa5]{1,6})', 'g');
  let m;
  while ((m = reA.exec(text)) !== null) {
    const f = trailingCategory(m[3]) ? null : flowerByText(m[3], flowers);
    if (f && !seen.has(f.id)) { seen.add(f.id); out.push({ flower_id: f.id, qty: parseInt(m[1], 10), unit: m[2] || null }); }
  }
  const reB = new RegExp('([\\u4e00-\\u9fa5]{1,6})\\s*(\\d+)\\s*(' + units + ')?', 'g');
  while ((m = reB.exec(text)) !== null) {
    const f = trailingCategory(m[1]) ? null : flowerByText(m[1], flowers);
    if (f && !seen.has(f.id)) { seen.add(f.id); out.push({ flower_id: f.id, qty: parseInt(m[2], 10), unit: m[3] || null }); }
  }
  // 中文数字：如“十三支百合”“一百零一朵玫瑰”
  const reC = new RegExp('([零一二两三四五六七八九十百千]+)\\s*(' + units + ')\\s*([\\u4e00-\\u9fa5]{1,6})', 'g');
  while ((m = reC.exec(text)) !== null) {
    const f = trailingCategory(m[3]) ? null : flowerByText(m[3], flowers);
    const qty = cn2num(m[1]);
    if (f && qty != null && !seen.has(f.id)) { seen.add(f.id); out.push({ flower_id: f.id, qty, unit: m[2] || null }); }
  }
  return out;
}
